...
export abstract class ValidatorProvider<P extends KernelBaseValidatorParams> extends ZeroDevProvider {
    ...
    constructor(params: ValidatorProviderParams<P>, validatorType: SupportedValidators = "ECDSA") {
        ...
        super({ ...params.opts?.providerConfig, rpcUrl: params.opts?.providerConfig?.rpcUrl, projectId: params.projectId, chain });
    ...
    sendEnableUserOperation = async (enableData: Hex): Promise<SendUserOperationResult> => {
        ...
        return await this.sendUserOperation({ target: this.account.validator.validatorAddress, data: encodedEnableData });
    };
    ...
    sendDisableUserOperation = async (disableData: Hex): Promise<SendUserOperationResult> => {
        ...
        return await this.sendUserOperation({ target: this.account.validator.validatorAddress, data: encodedDisableData });
    };
