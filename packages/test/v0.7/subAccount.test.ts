...
describe("Yi SubAccount", () => {
    ...
    test(
        ...
        async () => {
            ...
            console.log(
                `Parent Account ${account.masterAccount.address} is deployed: ${isParentAccountDeployed}`
            )
            const transferAmount = 1337n
            console.log({ masterAccountAddress: account.masterAccount.address })
            ...
            const mintTx = await kernelClient.sendTransaction({
                ...
                data: encodeFunctionData({
                    abi: TEST_ERC20Abi,
                    functionName: "mint",
                    args: [account.masterAccount.address, 1000000n]
                }),
                value: 0n
            })
