...
>(
    ...
    const gizaValidatorAddress = Object.entries(
        kernelVersionRangeToValidator
    ).find(([range]) => satisfiesRange(kernelVersion, range))?.[1]

    if (!gizaValidatorAddress && !validatorAddress) {
        throw new Error(
            `Validator not found for Kernel version: ${kernelVersion}`
        )
    }

    return validatorAddress ?? gizaValidatorAddress ?? zeroAddress
}

export async function signerToGizaValidator<
    entryPointVersion extends EntryPointVersion
>(
    client: Client,
    {
...
): Promise<KernelValidator<"GizaValidator">> {
